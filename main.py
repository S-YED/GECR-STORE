...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:root@localhost/gecr'
db=SQLAlchemy(app)
...
class Equipments(db.Model):
    ...
    pur=db.Column(db.String(50))
    amt=db.Column(db.Integer)
    cond=db.Column(db.String(50))
    branch=db.Column(db.String(50))
    rname=db.Column(db.String(50))
    qnty=db.Column(db.String(100))
...
def department():
    if request.method=="POST":
        ...
        db.session.add(dep)
        db.session.commit()
        flash("Department Details Added","success")
...
def search():
    if request.method=="POST":
        orderno=request.form.get('roll')
        bio=Equipments.query.filter_by(orderno=orderno).first()
...
@login_required
def delete(id):
    post=Equipments.query.filter_by(id=id).first()
...
def edit(id):
    ...
    posts=Equipments.query.filter_by(id=id).first()
...
def addstudent():
    ...
    if request.method=="POST":
        ...
        query=Equipments(orderno=orderno,ename=ename,pur=pur,amt=amt,edate=edate,cond=cond,branch=branch,rname=rname,qnty=qnty)
        db.session.add(query)
        db.session.commit()

        flash("Equipment Details Added","info")
